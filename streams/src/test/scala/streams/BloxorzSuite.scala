...
class BloxorzSuite extends FunSuite {
  ...
  test("bfs check") {
    new Level1 {
      val paths = pathsFromStart.take(100).toList
      val lengths = paths.foldRight(List[Int]())((x, y) => x._2.length :: y)
      assert(paths.length == 100 && lengths == lengths.sorted && pathsFromStart.filter(x => x._1 == Block(goal, goal)).length > 0)
    }
  }
